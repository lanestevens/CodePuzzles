...
candidate = sorted([(max(y), y, x) for x, y in parsed_data.items()])[-1]
selected_minute = sorted([(y, x) for x, y in enumerate(candidate[1])])[-1][1]
print candidate[2], selected_minute, candidate[2] * selected_minute
